...
const ORG_NAME = 'nitrobolt';
const SOURCE_LOCALE = 'en';

const tokenFile = pathUtil.join(__dirname, '..', '.tx_token');
const txToken = process.env.TX_TOKEN || (fs.existsSync(tokenFile) ? fs.readFileSync(tokenFile, 'utf-8').trim() : '');
if (!txToken) {
    throw new Error('TX_TOKEN is not defined and .tx_token does not exist.');
}
transifexApi.setup({
    auth: txToken
});
